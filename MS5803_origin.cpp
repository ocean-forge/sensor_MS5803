//
// Created by raphael on 2025-01-26
//
#include <Wire.h>
...
#define CMD_ADC_UP1		0x00	// ADC UP1 conversion
#define CMD_ADC_UT1		0x10	// ADC UT1 conversion
...
// UP1 and UT1 need to be unsigned 32-bit integers (long 0-4294967295)
static uint32_t     UP1 = 0;    // Store uncompensated pressure value
static uint32_t     UT1 = 0;    // Store uncompensated temperature value
...
boolean MS_5803::initializeMS_5803(boolean Verbose) {
    Wire.begin();
    // Check that the sensor is indeed here..
    Wire.beginTransmission(MS5803_I2C_ADDRESS);
    error = Wire.endTransmission();

    if (error == 0)
    {
        Serial.print("I2C device found at adress: ");
        Serial.println(MS5803_I2C_ADDRESS);
    }
    else
    {
        Serial.print("error ");
        Serial.println(error);
        Serial.print("I2C device not found at adress: ");
        Serial.println(MS5803_I2C_ADDRESS);
        while(1){
            ;
        }
...
void MS_5803::readSensor() {
    ...
    // to read.
    if (_Resolution == 256){
        UP1 = MS_5803_ADC(CMD_ADC_UP1 + CMD_ADC_256); // read raw pressure
        UT1 = MS_5803_ADC(CMD_ADC_UT1 + CMD_ADC_256); // read raw temperature
    } else if (_Resolution == 512) {
        UP1 = MS_5803_ADC(CMD_ADC_UP1 + CMD_ADC_512); // read raw pressure
        UT1 = MS_5803_ADC(CMD_ADC_UT1 + CMD_ADC_512); // read raw temperature
    } else if (_Resolution == 1024) {
        UP1 = MS_5803_ADC(CMD_ADC_UP1 + CMD_ADC_1024); // read raw pressure
        UT1 = MS_5803_ADC(CMD_ADC_UT1 + CMD_ADC_1024); // read raw temperature
    } else if (_Resolution == 2048) {
        UP1 = MS_5803_ADC(CMD_ADC_UP1 + CMD_ADC_2048); // read raw pressure
        UT1 = MS_5803_ADC(CMD_ADC_UT1 + CMD_ADC_2048); // read raw temperature
    } else if (_Resolution == 4096) {
        UP1 = MS_5803_ADC(CMD_ADC_UP1 + CMD_ADC_4096); // read raw pressure
        UT1 = MS_5803_ADC(CMD_ADC_UT1 + CMD_ADC_4096); // read raw temperature
    }
    // Calculate 1st order temperature, dT is a long integer
    // UT1 is originally cast as an uint32_t, but can fit in a int32_t, so we'll
    // cast both parts of the equation below as signed values so that we can
    // get a negative answer if needed
    dT = (int32_t)UT1 - ( (int32_t)sensorCoeffs[5] * 256 );
...
//    TEMP = 2000 + ((int64_t)dT * sensorCoeffs[6]) / (int64_t)1<<23;
    ...
    // float (mbar).
    // For 14 bar sensor
    mbarInt = ((UP1 * Sensitivity) / 2097152 - Offset) / 32768;
...
unsigned long MS_5803::MS_5803_ADC(char commandADC) {
    // UP1 and UT1 will come back as 24-bit values, and so they must be stored in
